...
public class FilterTasksByStatusCommand implements Command {
    ...
    public void execute() {
        ...
        Task.Status status;
        try { status = Task.Status.fromString(statusInput);
        } catch (IllegalArgumentException e) { view.showMessage("❌ Status inválido. Tente novamente.");
            return; }

        Optional<Comparator<Task>> orderBy = getSortingMethod();
        List<Task> tasks = taskController.getTasksByStatus(status, orderBy);

        if (tasks.isEmpty()) { view.showMessage("📭 Nenhuma tarefa encontrada com esse status.");
        } else { tasks.forEach(task -> view.showMessage(task.toString())); }
    }
    ...
    private Optional<Comparator<Task>> getSortingMethod() {
        ...
        if (option == 4) { return Optional.empty(); }
