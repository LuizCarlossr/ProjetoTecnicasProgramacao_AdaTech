...
public class TaskControllerImpl extends AbstractTaskController{
    ...
    public TaskControllerImpl(TaskService taskService) { super(taskService); }
    ...
    protected void validateDeadline(String deadline) {
        Optional.ofNullable(deadline)
                .map(date -> {
                    ...
                    } catch (DateTimeParseException e) { throw new IllegalArgumentException("Formato de data inválido. Use dd/MM/yyyy."); }
                })
    ...
    protected void validateStatus(Task.Status status) {
        ...
        if (status == Task.Status.CONCLUIDO) { throw new IllegalArgumentException("Uma nova tarefa não pode ser criada com o status 'CONCLUÍDO'."); }
    }
}