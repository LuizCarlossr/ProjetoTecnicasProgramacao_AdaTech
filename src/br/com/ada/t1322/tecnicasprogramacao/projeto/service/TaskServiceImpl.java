...
public class TaskServiceImpl extends AbstractTaskService {
    ...
    public TaskServiceImpl(TaskRepository taskRepository) { super(taskRepository); }
    ...
    public List<Task> findBy(Predicate<Task> predicate, Optional<Comparator<Task>> orderBy) {
        ...
        if (orderBy.isPresent()) { stream = stream.sorted(orderBy.get()); }
        return stream.toList();
    }

    @Override
    public void validate(Task task) {
        if (task == null) { throw new IllegalArgumentException("Task não pode ser nula"); }
